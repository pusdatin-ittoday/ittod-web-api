...
const upsertSubmissionController = async (req, res) => {
    try {
        ...
        const finalObject = typeof submission_object === 'string' ? JSON.parse(submission_object) : submission_object;

        const result = await upsertTeamSubmission(team_id, finalObject);
